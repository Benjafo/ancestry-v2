...
ALTER TABLE persons DROP CONSTRAINT IF EXISTS check_birth_before_death;
ALTER TABLE persons
ADD CONSTRAINT check_birth_before_death
CHECK (birth_date IS NULL OR death_date IS NULL OR birth_date < death_date);

ALTER TABLE persons DROP CONSTRAINT IF EXISTS check_birth_not_future;
ALTER TABLE persons
ADD CONSTRAINT check_birth_not_future
CHECK (birth_date IS NULL OR birth_date <= CURRENT_DATE);

ALTER TABLE persons DROP CONSTRAINT IF EXISTS check_death_not_future;
ALTER TABLE persons
ADD CONSTRAINT check_death_not_future
CHECK (death_date IS NULL OR death_date <= CURRENT_DATE);

ALTER TABLE persons DROP CONSTRAINT IF EXISTS check_gender_values;
ALTER TABLE persons
...
ALTER TABLE relationships DROP CONSTRAINT IF EXISTS check_start_before_end;
ALTER TABLE relationships
ADD CONSTRAINT check_start_before_end
CHECK (start_date IS NULL OR end_date IS NULL OR start_date < end_date);

ALTER TABLE relationships DROP CONSTRAINT IF EXISTS check_relationship_type;
ALTER TABLE relationships
...
ALTER TABLE relationships DROP CONSTRAINT IF EXISTS check_relationship_qualifier;
ALTER TABLE relationships
...
ALTER TABLE relationships DROP CONSTRAINT IF EXISTS check_different_persons;
ALTER TABLE relationships
ADD CONSTRAINT check_different_persons
CHECK (person1_id != person2_id);

-- Add check constraints to events table
ALTER TABLE events DROP CONSTRAINT IF EXISTS check_event_date_not_future;
ALTER TABLE events
...
ALTER TABLE project_users DROP CONSTRAINT IF EXISTS check_access_level;
ALTER TABLE project_users
ADD CONSTRAINT check_access_level
CHECK (access_level IN ('view', 'edit'));

-- Add check constraints to projects table
ALTER TABLE projects DROP CONSTRAINT IF EXISTS check_project_status;
ALTER TABLE projects
ADD CONSTRAINT check_project_status
CHECK (status IN ('active', 'completed', 'on_hold'));

-- Add check constraints to user_events table
ALTER TABLE user_events DROP CONSTRAINT IF EXISTS check_user_event_type;
